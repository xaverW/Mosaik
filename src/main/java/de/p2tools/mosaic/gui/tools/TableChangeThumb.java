    ...
    private ContextMenu getMenu() {
        ...
        if (thumbCollection != null && !thumbs.isEmpty()) {
            ...
            delThumb.setOnAction(a -> {
                if (thumbs.size() == 1 &&
                        !new PAlert().showAlertOkCancel("Datei Löschen?", "", "Die Datei löschen: " + P2LibConst.LINE_SEPARATORx2 + thumbs.get(0).getFileName())) {
                    return;

                } else if (thumbs.size() > 1 &&
                        !new PAlert().showAlertOkCancel("Dateien Löschen?", thumbs.size() + " Dateien löschen",
