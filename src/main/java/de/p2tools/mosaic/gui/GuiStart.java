...
public class GuiStart extends AnchorPane {
    ...
    private void initCont() {
        ...
        btnDestDir.setOnAction(event -> {
            if (!new PAlert().showAlert_yes_no("Pfad ändern", "Projekt verschieben?", "Soll das Projekt von: " + P2LibConst.LINE_SEPARATOR +
                    txtDir.getText() + P2LibConst.LINE_SEPARATORx2 +
...
//        gridPaneDest.add(lblPath, 0, ++row, 4, 1);
    ...
    private void moveProject() {
        ...
        dir = PDirFileChooser.DirChooser(ProgData.getInstance().primaryStage, dir).trim();
    ...
    private void addProjectDataDialog() {
        AddMosaicDialogController amc = new AddMosaicDialogController();
        ProjectData pd = amc.getProjectData();

        if (pd == null && progData.projectDataList.isEmpty()) {
            pd = new ProjectData();
