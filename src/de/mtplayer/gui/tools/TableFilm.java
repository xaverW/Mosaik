...
import de.mtplayer.controller.data.film.FilmSize;
...
public class TableFilm {
    ...
    private final StringProperty geoLocation;

    public TableFilm(Daten daten) {
        this.daten = daten;
        geoMelden = Config.SYSTEM_GEO_MARK.getBooleanProperty();
        geoLocation = Config.SYSTEM_GEO_HOME_PLACE.getStringProperty();
    }
    ...
    public TableColumn[] initFilmColumn(TableView table) {
        ...
        final TableColumn<Film, Datum> datumColumn = new TableColumn<>("Datum");
        ...
        return new TableColumn[]{
                ...
                urlColumn, aboColumn, newColumn, shownColumn,
        ...
        table.setRowFactory(tableview -> new TableRow<Film>() {
            ...
            public void updateItem(Film film, boolean empty) {
                ...
                if (film == null || empty) {
                    ...
                    } else if (geoMelden.get() && !film.getGeo().isEmpty() && !film.getGeo().contains(geoLocation.get())) {
                        // geogeblockt
                        for (int i = 0; i < getChildren().size(); i++) {
