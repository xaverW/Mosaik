...
public class FilmlistBlackFilter {
    ...
    private static boolean checkGeoBlockedFilm(Film film) {
        boolean result = true;
        if (!film.arr[FilmXml.FILM_GEO].isEmpty()
                && !film.arr[FilmXml.FILM_GEO].contains(Config.SYSTEM_GEO_HOME_PLACE.get())) {
            result = false;
        }
