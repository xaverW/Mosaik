...
package de.mtplayer.tools.storedFilter;
...
import de.mtplayer.tools.filmListFilter.FilmFilter;
...
public final class SelectedFilter extends SelectedFilterProps {
    ...
    public void clearFilter() {
        ...
        setDays(SelectedFilter.FILTER_DAYS_MAX);

        setMinDur(0);
        setMaxDur(SelectedFilter.FILTER_DURATIION_MAX_MIN);

        setMinTime(0);
        setMaxTime(SelectedFilter.FILTER_FILMTIME_MAX_SEC);
    ...
    public Predicate<Film> getPred() {
        ...
        final int maxLaengeSec = selectedFilter.getMinMaxDurVis() ? selectedFilter.getMaxDur() * 60 : SelectedFilter.FILTER_DURATIION_MAX_SEC;

        // Filmzeit in Sek. von 0:00 Uhr
        final int minTimeSec = selectedFilter.isMinMaxTimeVis() ? selectedFilter.getMinTime() : 0;
        final int maxTimeSec = selectedFilter.isMinMaxTimeVis() ? selectedFilter.getMaxTime() : SelectedFilter.FILTER_FILMTIME_MAX_SEC;
        final boolean minMaxTimeInvert = selectedFilter.getMinMaxTimeInvert();

        long days = 0;
        try {
            if (selectedFilter.getDays() == SelectedFilter.FILTER_DAYS_MAX) {
                days = 0;
            } else {
        ...
        if (maxLaengeSec != SelectedFilter.FILTER_DURATIION_MAX_SEC) {
            predicate = predicate.and(f -> FilmFilter.checkLengthMax(maxLaengeSec, f.dauerL));
        }

        // Film-Uhrzeit
        if (minTimeSec != 0 || maxTimeSec != SelectedFilter.FILTER_FILMTIME_MAX_SEC) {
