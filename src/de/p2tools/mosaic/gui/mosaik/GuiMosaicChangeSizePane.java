...
public class GuiMosaicChangeSizePane extends AnchorPane {
    ...
    public void isShown() {
        ...
        if (!mosaicData.equals(progData.selectedProjectData.getMosaicData())) {
            unbind();
            mosaicData = progData.selectedProjectData.getMosaicData();
    ...
    private void initCont() {
        ...
        rbNon.setOnAction(e -> mosaicData.setResizeThumb(MosaicData.THUMB_RESIZE.NON.toString()));
        rbAll.setOnAction(e -> mosaicData.setResizeThumb(MosaicData.THUMB_RESIZE.ALL.toString()));
        rbDark.setOnAction(e -> mosaicData.setResizeThumb(MosaicData.THUMB_RESIZE.DARK.toString()));
        rbLight.setOnAction(e -> mosaicData.setResizeThumb(MosaicData.THUMB_RESIZE.LIGHT.toString()));
    ...
    private void unbind() {
        ...
        // Resize
        iPropSize.unbind();
        mosaicData.borderSizeProperty().unbind();
    ...
    private void bind() {
        ...
        if (mosaicData.getResizeThumb().equals(MosaicData.THUMB_RESIZE.ALL.toString())) {
            rbAll.setSelected(true);
        } else if (mosaicData.getResizeThumb().equals(MosaicData.THUMB_RESIZE.DARK.toString())) {
            rbDark.setSelected(true);
        } else if (mosaicData.getResizeThumb().equals(MosaicData.THUMB_RESIZE.LIGHT.toString())) {
        ...
        // Resize
        sliderSize.setValue(mosaicData.getBorderSize() / 2);
        iPropSize.bind(sliderSize.valueProperty());

        NumberBinding nb = Bindings.multiply(iPropSize, 2);
        mosaicData.borderSizeProperty().bind(nb);

        lblSlider.textProperty().bind(Bindings.format("%d", mosaicData.borderSizeProperty()));
    }
}
